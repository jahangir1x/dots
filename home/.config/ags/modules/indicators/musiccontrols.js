  ...
  const realCoverArt = Box({
    ...
    attribute: {
      ...
      updateCover: (self) => {
        ...
        const stylePath = `${player.coverPath}${
          darkMode.value ? "" : "-l"
        }${COVER_COLORSCHEME_SUFFIX}`;
        ...
        execAsync([
          ...
          } > ${GLib.get_user_state_dir()}/ags/scss/_musicmaterial.scss`,
        ])
          .then(() => {
            exec(
              `wal -i "${player.coverPath}" -n -t -s -e -q ${
                darkMode.value ? "" : "-l"
              }`
            );
            exec(
