...
class BrightnessDdcService extends BrightnessServiceBase {
    ...
    setBrightnessCmd(percent) {
        return `bash -c "ddcutil -b ${this._busNum} setvcp 10 ${Math.round(percent * 100)}; ddcutil -b ${this._busNum} setvcp 12 ${Math.round(percent * 100)}"`;
    }
}
...
async function listDdcMonitorsSnBus() {
    let ddcSnBus = {};
    try {
        const out = await Utils.execAsync(`bash -c "ddcutil detect --brief | grep -v Unable"`);
